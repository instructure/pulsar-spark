...
private[pulsar] case class PulsarMetadataReader(
  ...
  def forwardOffset(actualOffset: Option[Map[String, MessageId]],
                    ...
                    minEntriesPerTopic: Long): SpecificPulsarOffset = {
    getTopicPartitions()
    ...
    val forwarder = strategy match {
      case PulsarOptions.PROPORTIONAL_FORWARD_STRATEGY =>
        new ProportionalForwardStrategy(numberOfEntriesToForward, minEntriesPerTopic)
      case PulsarOptions.LARGE_FIRST_FORWARD_STRATEGY =>
        new LargeFirstForwardStrategy(numberOfEntriesToForward, minEntriesPerTopic)
      case _ =>
    ...
    SpecificPulsarOffset(topicPartitions.map { topic =>
      topic -> PulsarSourceUtils.seekableLatestMid {
        ...
        try {
          ...
          // Log state
          val forwardedEntry = TopicInternalStatsUtils.numOfMessagesUntil(
